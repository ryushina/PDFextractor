...
def analyze_table_data(table_data):
    ...
    response = client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": "You are a data analyst that extracts from pdf files"},
                  {"role": "user", "content": f"Extract fields from the given text then output python dictionary or list of dictionaries if there are more than one asset:Asset Name| Filename| Delivery Date| City| Country| Start of Lease| Tenant| GLA| IP-Rent| Start of Contract| with Text:{table_data}"},
                  {"role": "assistant", "content": """
    Result should just be just valid json string or an empty json if no meaningful data found
    The fields are:
